...
public:
    ...
    * together.
    */
    const(void)[] uncompress(const(void)[] buf, size_t outSize)
    in
    {
        assert(!done);
    }
...
unittest // by Dave
{
    debug(zlib) writeln("util.zlib.unittest");

    bool CompressThenUncompress (ubyte[] src)
    {
        try {
            ubyte[] dst = cast(ubyte[])util.zlib.compress(cast(void[])src);
            ...
            uncompressedBuf = cast(ubyte[])util.zlib.uncompress(cast(void[])dst);
    ...
    debug(zlib) writefln("PASSED util.zlib.unittest");
}
...
unittest // by Artem Rebrov
{
    ...
    buf ~= cmp.flush();
    const(void)[] output = decmp.uncompress(buf, input.length);
